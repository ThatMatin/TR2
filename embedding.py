...
class PeptidePrecursorEmbedding(nn.Module):
    ...
    def forward(self, y:torch.Tensor, charge:torch.Tensor, mz:torch.Tensor):
        ...
        mz_emb = self.mz_emb( mz )
        
        if len( charge_emb.shape )<3:
            charge_emb = charge_emb.unsqueeze(1)

        input_seq = torch.cat( [pep_emb + charge_emb, mz_emb], dim=1 )
        return input_seq
